...
app.post('/api/auth/login', (req, res) => {
  ...
  const user = db.prepare('SELECT * FROM users WHERE login_id = ? OR email = ?').get(loginId.trim(), loginId.trim().toLowerCase())
...
const hasLockedScrutinyReview = (appId) => {
...
app.get('/api/scrutiny/queue', scrutinyAuth, (req, res) => {
  ...
  const rows = db.prepare(`
    ...
    WHERE a.status IN ('Submitted', 'Under Scrutiny')
...
app.patch('/api/scrutiny/applications/:id/documents/:docId', scrutinyAuth, (req, res) => {
  ...
  if (hasLockedScrutinyReview(appRow.id) || ['Referred', 'Finalized'].includes(appRow.status)) {
...
app.post('/api/scrutiny/applications/:id/gist/generate', scrutinyAuth, (req, res) => {
  ...
  if (!tpl) return res.status(400).json({ message: `No admin template configured for category ${appRow.category}.` })
  ...
  const gistText = [
    `PARIVESH Meeting Gist (${appRow.category})`,
    ...
    '',
    'Sector Parameters:',
    JSON.stringify(sectorParams, null, 2),
    '',
    `Template Used: ${tpl.name} (${tpl.original_name})`,
    `Generated By: ${req.user.login_id}`,
    `Generated At: ${new Date().toISOString()}`,
  ].join('\n')

  const stamp = `${Date.now()}_${appRow.id}`
  const docxFile = `gist_${stamp}.docx`
  const pdfFile = `gist_${stamp}.pdf`
  const docxPath = path.join(uploadsDir, docxFile)
  const pdfPath = path.join(uploadsDir, pdfFile)
  fs.writeFileSync(docxPath, gistText, 'utf8')
  fs.writeFileSync(pdfPath, gistText, 'utf8')
  ...
  if (existing) {
    ...
    db.prepare('UPDATE scrutiny_gists SET docx_path = ?, pdf_path = ?, generated_by = ?, generated_at = datetime(\'now\') WHERE application_id = ?')
      .run(docxFile, pdfFile, req.user.login_id, appRow.id)
  } else {
    db.prepare('INSERT INTO scrutiny_gists (application_id, docx_path, pdf_path, generated_by, generated_at) VALUES (?, ?, ?, ?, datetime(\'now\'))')
      .run(appRow.id, docxFile, pdfFile, req.user.login_id)
  }

  db.prepare('UPDATE applications SET status = ?, updated_at = datetime(\'now\') WHERE id = ?').run('Under Scrutiny', appRow.id)
  db.prepare('INSERT INTO application_status_history (application_id, status, comment, created_by) VALUES (?, ?, ?, ?)')
    .run(appRow.id, 'Under Scrutiny', 'Meeting gist generated by scrutiny team', req.user.login_id)
...
app.get('/api/mom/applications/:id/gist', momAuth, (req, res) => {
  ...
  if (gistRow) {
    ...
    if (fs.existsSync(docxPath)) {
      ...
      return res.json({ content, source: 'scrutiny_gist' })
    }
  }
...
app.post('/api/mom/applications/:id/convert', momAuth, (req, res) => {
  const appRow = db.prepare(`
    SELECT a.*, s.name as sector_name, u.full_name as owner_name
  ...
  const stamp = `${Date.now()}_${appRow.id}`
  const docxFile = `mom_${stamp}.docx`
  const pdfFile = `mom_${stamp}.pdf`
  fs.writeFileSync(path.join(uploadsDir, docxFile), momText, 'utf8')
  fs.writeFileSync(path.join(uploadsDir, pdfFile), momText, 'utf8')
  ...
  if (existing) {
    db.prepare(`UPDATE mom_records
      ...
      docxFile, pdfFile, req.user.login_id, now, appRow.id
    )
  } else {
    db.prepare(`INSERT INTO mom_records
      ...
      docxFile, pdfFile, req.user.login_id, now
    )
  }
